...
class LanguageView extends View
{
	 ...
	 */
	public function __construct(array $path, array $items = array(), $lang = null)
	{
		$language = isset($lang) ? $lang : Lang::defaultInstance()->locale;
		$this->fullPath = join(DS, array_merge(['public', 'views', $language], $path)) . EXT;
		if (!file_exists($this->fullPath)) {
			$this->fullPath = join(DS, array_merge(['public', 'views', Lang::defaultInstance()->defaultLocale], $path)) . EXT;
			if (!file_exists($this->fullPath)) {
